...
def classify_intent(state: AgentState, config: RunnableConfig) -> AgentState:
    ...
    prompt = get_intent_classification_prompt().format(conversation_history=history, user_input=state.get("user_input", ""))

    try:
        llm_response = llm.invoke(prompt)
    except Exception as e:
        # If there's an error during intent classification, log it and default to "unknown"
        print(f"Error during intent classification: {e}")
        llm_response = UserIntent(intent_type="unknown", confidence=0.0, reasoning=str(e))

    if llm_response.intent_type == "qa":
        next_step = "qa"
    elif llm_response.intent_type == "summarization":
        next_step = "summarization"
    elif llm_response.intent_type == "calculation":
        next_step = "calculation"
    else:
        next_step = "qa"  # Default to QA agent for unknown intent
...
def update_memory(state: AgentState, config: RunnableConfig) -> AgentState:
    ...
    llm = config.get("configurable").get("llm")  # Replace with actual retrieval of LLM from config
